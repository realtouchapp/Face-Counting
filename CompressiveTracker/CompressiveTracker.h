...
private:
	...
	void radioClassifier_1(vector<float>& _muPos, vector<float>& _sigmaPos, vector<float>& _muNeg, vector<float>& _sigmaNeg,
		Mat& _sampleFeatureValue, float& _radioMax, int& _radioMaxIndex);
...
public:
	...
	float processFrameTest(Mat& _frame, Rect& _objectBox);
	float ProcessFrameWithScore(Mat& _frame, Rect& _objectBox);
	void init_Test(Mat& _frame, Rect& _objectBox);

	void CT_SetParameter(
